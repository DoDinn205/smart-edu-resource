...
public class StudentLearningServiceImpl implements StudentLearningService {
    ...
    @Override
    public LearningLog startLearning(String username, int resourceId) {
        Resource resource = this.resourcementRepo.getResourceById(resourceId);

        if (resource == null) {
            throw new RuntimeException("Không tìm thấy học liệu!");
        }

        List<Enrollment> enrollments = this.enrollmentRepo.getEnrollmentsByUsername(username);

        if (enrollments == null || enrollments.isEmpty()) {
            throw new RuntimeException("Bạn chưa đăng ký khóa học nào!");
        }
        ...
        return this.learningLogRepo.addLearningLog(log);
    }

    @Override
    public LearningLog completeLearning(String username, int resourceId) {
        Resource resource = this.resourcementRepo.getResourceById(resourceId);
    ...
    @Override
    public List<LearningLog> getHistory(String username) {
